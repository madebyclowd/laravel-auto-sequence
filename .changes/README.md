...
multiple PRs are in flight) or relying on commit-message parsing (which
this project tried once, via `release-please`, and moved away from).

## Adding one

For any user-facing change, copy `TEMPLATE.md` to a new file in this
directory named after your change (e.g. `fix-audit-fillable.md` — the
filename doesn't matter beyond being unique and ending in `.md`), then
...
Audit trail (`created_by`/`updated_by`) was silently never persisted because
`Models\Sequence::$fillable` didn't include the audit columns.
```
