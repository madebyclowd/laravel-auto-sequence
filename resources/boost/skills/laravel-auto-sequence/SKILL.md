...
- **FPM Worker Exhaustion & Database Locking**: The `database` locking driver sets a native database session/local lock wait timeout based on `locking.timeout` (default 5s) instead of blocking the PHP execution thread indefinitely. Avoid long-running transactions to prevent fast-failing lock acquisition exceptions.
- **`load_migrations => false` in tests**: The package's bundled migrations now respect this config during your app's own test suite too. If your app ships a replacement `sequences` table migration under its own name and sets `load_migrations => false`, that's honored in `phpunit`/`php artisan test` runs, not just in normal/prod environments.
